...
public class MITEFurnaceContainer extends FurnaceContainer {
	...
	protected boolean func_217057_a(ItemStack p_217057_1_) {
		...
		Field recipeType;
		try {
			recipeType = AbstractFurnaceContainer.class.getDeclaredField(FiveMinSurvival.DEBUG ? "recipeType" : "field_214014_c");
			try {
				Resources.makeFieldAccessible(recipeType);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			try {
				boolean b = this.world.getRecipeManager().getRecipe((IRecipeType)recipeType.get(this), new Inventory(p_217057_1_), this.world).isPresent();
				return b;
   ...
   public int getCookProgressionScaled() {
	   try {
		   ...
		   try {
			   Resources.makeFieldAccessible(field_217064_e);
		   }catch (Exception e) {e.printStackTrace();}
	      int i = ((IIntArray)field_217064_e.get(this)).get(2);
	      int j = ((IIntArray)field_217064_e.get(this)).get(3);
